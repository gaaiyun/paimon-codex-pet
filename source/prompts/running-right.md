Create a single horizontal sprite strip for the Codex app digital pet `paimon` in the state `running-right`.
...
Output exactly 8 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: A tiny floating chibi companion inspired by Paimon from Genshin Impact: white bobbed hair, small golden halo-like crown, star-shaped hair accents, navy-and-white outfit with gold details, cheerful expressive face, tiny limbs, compact readable silhouette. Adapt into Codex digital pet style, not polished anime key art..
